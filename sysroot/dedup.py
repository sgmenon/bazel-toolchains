...
def scan_directory(
    root_dir: Path, follow_symlinks: bool = False
) -> Dict[str, List[Path]]:
    """
    ...
    for dirpath, _dirnames, filenames in os.walk(root_dir, followlinks=follow_symlinks):
...
def find_duplicates(
    ...
    for _file_hash, file_list in hash_to_files.items():
        if len(file_list) > 1:
...
def replace_with_symlink(
    original: Path, duplicate: Path, dry_run: bool = False
) -> bool:
            ...
            for copy in copies:
                ...
                if args.absolute_links:
                    ...
                    try:
                        if args.dry_run:
                            print(
                                f"Would create absolute symlink: {copy} -> {original.absolute()}"
                            )
                            success = True
                        else:
                            copy.unlink()
